...
class GetExchangeListUseCaseTest {
    ...
    private lateinit var useCaseTest: GetExchangeListUseCase

    @Before
    fun setUp() {
        MockKAnnotations.init(this)
        useCaseTest = GetExchangeListUseCase(
            exchangesRepository,
            gson
        )
    }
    ...
    @Test
    fun `should get exchange list when execute the use case`() = runBlocking {
        ...
        coEvery { exchangesRepository.getExchanges() } returns Response.success(
        ...
        coVerify { exchangesRepository.getExchanges() }
    ...
    fun `should get failed response when execute the use case`() = runBlocking {
        ...
        val codeMock = 404
        val messageMock = "{ \"message\": \"Not found\", \"code\": 404 }".toResponseBody("application/json".toMediaTypeOrNull())
        val responseMock = "Not found"

        coEvery { exchangesRepository.getExchanges() } returns Response.error(
            codeMock,
            messageMock
        )
